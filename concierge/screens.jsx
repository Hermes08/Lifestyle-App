// screens.jsx v6 — Panama Lifestyle OS · Concierge Screens
// v5: BottomTabBar on all authenticated screens, ScreenChecklist, checklist
//     progress widget on Desk, session-state awareness.
// v6: ScreenProviders — provider directory with category filter, CRUD, add form.
...
// Status bar — left side: back chevron + screen label; right: tone glyph → account
    ...
    <div style={{
      ...
      {tabs.map(function (tab) {
        ...
        return (
          <div
            key={tab.id}
            onClick={() => window.AppState?.navigate(tab.id)}
            style={{
              flex: 1, display: 'flex', flexDirection: 'column',
              alignItems: 'center', justifyContent: 'center',
              gap: 5, cursor: 'pointer',
              borderTop: `2px solid ${isActive ? tone.accent : 'transparent'}`,
              paddingTop: 4,
              transition: 'border-color 0.15s',
            }}>
            <div style={{
              fontSize: 16, lineHeight: 1,
              color: isActive ? tone.accent : tone.mute,
...
function FollowUpBanner({ tone }) {
  const [followUp, setFollowUp] = React.useState(
    () => (window.AppState?.getState()?.followUp) || null
  );
  React.useEffect(() => {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${tone.accent}44`,
      borderLeft: `3px solid ${tone.accent}`,
    }}>
      <div style={{
        fontFamily: tone.serif, fontStyle: 'italic', fontSize: 17,
        color: tone.ink, lineHeight: 1.45,
      }}>{followUp.msg}</div>
      <div style={{ display: 'flex', gap: 12, marginTop: 10, alignItems: 'center' }}>
        <div
          onClick={() => {
            window.AppState?.markDone(followUp.id);
            window.AppState?.navigate(followUp.ctaScreen);
          }}
          style={{
            fontFamily: tone.mono, fontSize: 12, letterSpacing: '0.18em',
            textTransform: 'uppercase', background: tone.accent, color: tone.bg,
            padding: '9px 14px', cursor: 'pointer',
          }}>
          {followUp.ctaLabel}
        </div>
        <div
          onClick={() => window.AppState?.dismissFollowUp()}
          style={{
            fontFamily: tone.mono, fontSize: 11, letterSpacing: '0.14em',
            color: tone.mute, cursor: 'pointer',
          }}>
          LUEGO
        </div>
      </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          <button
            onClick={() => window.AppState?.navigate(isReturning ? 'desk' : 'picker')}
            style={{
            ...
            {isReturning ? 'Volver al escritorio' : (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
          {stages.map((s, i) => {
            ...
            return (
              <div key={s.id}
                onClick={() => {
                  window.AppState?.selectStage(s.id);
                  window.AppState?.confirmStage();
                  window.AppState?.navigate('desk');
                }}
                style={{
                  padding: '22px 22px',
...
/* ── Screen 3: Service desk ─────────────────────────────────────── */
function ScreenDesk({ tone }) {
  const [checklistItems, setChecklistItems] = useState(
    () => window.AppState?.getState()?.checklistItems || []
  );
  useEffect(() => {
    if (!window.AppState) return;
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, padding: '110px 24px 80px',
        ...
        {checklistTotal > 0 && (
          <div
            onClick={() => window.AppState?.navigate('checklist')}
            style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 2, background: tone.line, borderRadius: 1 }}>
                <div style={{
                  height: '100%', width: `${checklistPct}%`,
                  background: tone.accent, borderRadius: 1,
            ...
            }}>{checklistDone}/{checklistTotal} ›</div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1,
          ...
          {items.map((s, i) => (
            <div
              key={i}
              onClick={() => window.AppState?.openChat(s.l)}
              style={{
                padding: '22px 18px',
                background: tone.bg2,
...
function ScreenChat({ tone }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, padding: '108px 20px 148px',
      ...
      <div style={{
        position: 'absolute', left: 20, right: 20, bottom: 78,
...
function ScreenChecklist({ tone }) {
  ...
  const STATUS_GLYPH = {
    not_started:       { g: '○', accent: false },
    in_progress:       { g: '◑', accent: false },
    waiting_client:    { g: '◷', accent: false },
    waiting_concierge: { g: '◶', accent: false },
    completed:         { g: '●', accent: true  },
    optional:          { g: '◌', accent: false },
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{
            fontFamily: tone.mono, fontSize: 12, letterSpacing: '0.28em',
            color: tone.mute, textTransform: 'uppercase',
          }}>Your Roadmap</div>
          <div style={{
            fontFamily: tone.serif, fontWeight: 300, fontSize: 38,
            lineHeight: 1.1, letterSpacing: '-0.015em', marginTop: 6,
          }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <div style={{
              fontFamily: tone.mono, fontSize: 11, color: tone.mute,
              letterSpacing: '0.2em', textTransform: 'uppercase',
            }}>{completed} of {total} complete</div>
            <div style={{
              fontFamily: tone.mono, fontSize: 11, color: tone.accent,
          ...
          return (
            <div key={cat.id}>
              <div style={{
                ...
                <div style={{
                  fontFamily: tone.mono, fontSize: 10, color: tone.accent,
                  opacity: 0.7, letterSpacing: '0.14em',
                }}>{catDone}/{catItems.length}</div>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {catItems.map((item, i) => {
                  const sg    = STATUS_GLYPH[item.status] || STATUS_GLYPH.not_started;
                  ...
                  return (
                    <div
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: 14,
                        padding: '14px 0',
                        borderBottom: i < catItems.length - 1
                          ? `1px solid ${tone.line}44` : 'none',
                        cursor: 'pointer',
                        opacity: isDone ? 0.5 : 1,
                        transition: 'opacity 0.2s',
                      }}>
                      <div style={{
                        fontFamily: tone.mono, fontSize: 15, lineHeight: 1,
                        color: sg.accent ? tone.accent : tone.mute,
                        width: 18, textAlign: 'center', flexShrink: 0,
                      }}>{sg.g}</div>
                      <div style={{
                        fontFamily: tone.serif, fontSize: 19,
                        color: tone.ink, flex: 1, lineHeight: 1.3,
        ...
        <div style={{
          paddingTop: 14, borderTop: `1px solid ${tone.line}`,
          marginTop: 6,
        }}>
          <div
            onClick={() => window.AppState?.navigate('chat')}
            ...
            <div style={{
              fontFamily: tone.serif, fontStyle: 'italic', fontSize: 18,
              color: tone.mute, flex: 1, lineHeight: 1.4,
            }}>Need help with any step?</div>
...
function ScreenPricing({ tone }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, padding: '108px 22px 80px',
...
function ScreenAccount({ tone }) {
  return (
    <div style={{
      ...
      <div style={{
        position: 'absolute', inset: 0, padding: '108px 24px 80px',
        ...
        {[
          ...
          { glyph: '◧', label: 'Your Roadmap',     sub: 'Checklist & progress',       screen: 'checklist' },
          ...
          { glyph: '◆', label: 'Providers',         sub: 'Directory & vendor network', screen: 'providers' },
        ].map((item, i) => (
          <div
            key={i}
        ...
        <div style={{ marginTop: 'auto', paddingTop: 28 }}>
          <div
            ...
            style={{ padding: '16px 0', borderTop: `1px solid ${tone.line}`, cursor: 'pointer' }}>
            <div style={{
              fontFamily: tone.serif, fontStyle: 'italic', fontSize: 18,
              color: tone.mute,
            }}>Leave the room</div>
...
/* ── Screen 8: Provider Dashboard ───────────────────────────────── */
function ScreenProviders({ tone }) {
  const [providers, setProviders] = useState(() => window.AppState?.getState()?.providers || []);
  const [activeCat, setActiveCat] = useState('all');
  const [showAdd, setShowAdd] = useState(false);
  const [expandedId, setExpandedId] = useState(null);
  const [form, setForm] = useState({
    name: '', category: 'legal', specialty: '', phone: '', email: '', tier: 'all', notes: '',
  });

  useEffect(() => {
    if (!window.AppState) return;
    const unsub = window.AppState.subscribe(s => setProviders(s.providers || []));
    return () => unsub && unsub();
  }, []);
  ...
  const CAT_GLYPH = {
    legal: '⚖', realestate: '◆', health: '✚', leisure: '◉',
    transport: '▲', finance: '◈', lifestyle: '○', all: '·',
  };
  const TIER_LABEL = { all: 'All tiers', explorer: 'Explorer+', settler: 'Settler+', resident: 'Resident+', legend: 'Legend only' };

  const filtered = activeCat === 'all' ? providers : providers.filter(p => p.category === activeCat);

  function submitAdd() {
    if (!form.name.trim()) return;
    window.AppState?.addProvider({ ...form });
    setForm({ name: '', category: 'legal', specialty: '', phone: '', email: '', tier: 'all', notes: '' });
    setShowAdd(false);
  }

  const inputStyle = {
    width: '100%', padding: '10px 12px', boxSizing: 'border-box',
    background: `${tone.accent}08`, border: `1px solid ${tone.line}`,
    color: tone.ink, fontFamily: tone.sans, fontSize: 14, outline: 'none',
  };
  const labelStyle = {
    fontFamily: tone.mono, fontSize: 9, color: tone.mute,
    letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 5, display: 'block',
  };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', color: tone.ink, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 96, paddingBottom: 80 }}>
        <div style={{ padding: '0 24px 16px' }}>
          <div style={{ fontFamily: tone.mono, fontSize: 10, color: tone.mute, letterSpacing: '0.26em', textTransform: 'uppercase', marginBottom: 4 }}>Provider Directory</div>
          <div style={{ fontFamily: tone.serif, fontSize: 33, color: tone.ink, lineHeight: 1.15 }}>
            Your <em style={{ color: tone.accent, fontStyle: 'italic' }}>network</em>
          </div>
        </div>

        <div style={{ padding: '0 24px 16px', display: 'flex', gap: 7, overflowX: 'auto', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
          {CATS.map(cat => {
            ...
            return (
              <div key={cat.id} onClick={() => setActiveCat(cat.id)} style={{
                ...
                border: `1px solid ${active ? tone.accent : tone.line}`,
                color: active ? tone.bg : tone.mute,
                fontFamily: tone.mono, fontSize: 9, letterSpacing: '0.16em',
        ...
        <div style={{ padding: '0 24px 12px', fontFamily: tone.mono, fontSize: 10, color: tone.mute, letterSpacing: '0.18em', textTransform: 'uppercase' }}>
          {filtered.length} {filtered.length !== 1 ? 'providers' : 'provider'}
        </div>

        <div style={{ padding: '0 24px' }}>
          {filtered.length === 0 && (
            <div style={{ padding: '36px 0', textAlign: 'center', fontFamily: tone.sans, fontSize: 15, color: tone.mute, fontStyle: 'italic' }}>
              No providers in this category yet.
            </div>
          )}
          {filtered.map(p => (
            <div key={p.id} onClick={() => setExpandedId(expandedId === p.id ? null : p.id)} style={{
              marginBottom: 10, padding: '15px',
              background: tone.surface, border: `1px solid ${tone.line}`,
              borderLeft: `3px solid ${p.status === 'active' ? tone.accent : tone.line}`,
              cursor: 'pointer',
            }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <div style={{
                  width: 36, height: 36, borderRadius: '50%', flexShrink: 0,
                  background: `${tone.accent}1a`, border: `1px solid ${tone.accent}44`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: tone.mono, fontSize: 13, color: tone.accent,
                }}>{CAT_GLYPH[p.category] || '◈'}</div>

                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                    <span style={{ fontFamily: tone.serif, fontSize: 17, color: tone.ink }}>{p.name}</span>
                    <span style={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, display: 'inline-block', background: p.status === 'active' ? tone.accent : tone.line }}/>
                  </div>
                  <div style={{ fontFamily: tone.sans, fontSize: 13, color: tone.mute, marginBottom: 7 }}>{p.specialty}</div>
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: tone.mono, fontSize: 8, letterSpacing: '0.14em', textTransform: 'uppercase', padding: '2px 6px', background: `${tone.accent}18`, color: tone.accent, border: `1px solid ${tone.accent}33` }}>
                      {CATS.find(c => c.id === p.category)?.label || p.category}
                    </span>
                    <span style={{ fontFamily: tone.mono, fontSize: 8, letterSpacing: '0.14em', textTransform: 'uppercase', padding: '2px 6px', color: tone.mute, border: `1px solid ${tone.line}` }}>
                      {TIER_LABEL[p.tier] || p.tier}
                    </span>
                  </div>
                </div>
                <span style={{ fontFamily: tone.mono, fontSize: 12, color: `${tone.accent}88`, flexShrink: 0 }}>{expandedId === p.id ? '▲' : '▼'}</span>
              </div>

              {expandedId === p.id && (
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${tone.line}` }}>
                  {[['PHONE', p.phone], ['EMAIL', p.email], ['NOTES', p.notes]].map(([lbl, val]) => val ? (
                    <div key={lbl} style={{ display: 'flex', gap: 10, marginBottom: 8, alignItems: 'flex-start' }}>
                      <span style={{ fontFamily: tone.mono, fontSize: 9, color: tone.mute, letterSpacing: '0.14em', textTransform: 'uppercase', width: 48, flexShrink: 0, paddingTop: 2 }}>{lbl}</span>
                      <span style={{ fontFamily: tone.sans, fontSize: 13, color: tone.ink, wordBreak: 'break-all' }}>{val}</span>
                    </div>
                  ) : null)}
                  <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                    <div onClick={e => { e.stopPropagation(); window.AppState?.updateProvider(p.id, { status: p.status === 'active' ? 'inactive' : 'active' }); }} style={{ fontFamily: tone.mono, fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', padding: '7px 11px', border: `1px solid ${tone.line}`, color: tone.mute, cursor: 'pointer' }}>
                      {p.status === 'active' ? 'Deactivate' : 'Activate'}
                    </div>
                    <div onClick={e => { e.stopPropagation(); window.AppState?.navigate('chat'); }} style={{ fontFamily: tone.mono, fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', padding: '7px 11px', background: tone.accent, color: tone.bg, cursor: 'pointer' }}>
                      Message
                    </div>
                    <div onClick={e => { e.stopPropagation(); window.AppState?.deleteProvider(p.id); setExpandedId(null); }} style={{ fontFamily: tone.mono, fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', padding: '7px 11px', border: `1px solid ${tone.line}`, color: `${tone.mute}88`, cursor: 'pointer', marginLeft: 'auto' }}>
                      Remove
                    </div>
                  </div>
                </div>
        ...
        <div style={{ padding: '12px 24px 0' }}>
          <div onClick={() => setShowAdd(!showAdd)} style={{
            ...
            border: `1px dashed ${tone.accent}55`, cursor: 'pointer',
            background: showAdd ? `${tone.accent}0a` : 'transparent',
          }}>
            <span style={{ fontFamily: tone.mono, fontSize: 18, color: tone.accent, lineHeight: 1 }}>+</span>
            <span style={{ fontFamily: tone.mono, fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: tone.accent }}>Add Provider</span>
          </div>

          {showAdd && (
            <div style={{ padding: '16px', background: tone.surface, border: `1px solid ${tone.line}`, borderTop: 'none' }}>
              <div style={{ display: 'grid', gap: 12 }}>
                ...
                <div>
                  ...
                  <input value={form.specialty} onChange={e => setForm(f => ({...f, specialty: e.target.value}))} placeholder="e.g. Tax Law · 12 yrs" style={inputStyle}/>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div>
                    <label style={labelStyle}>Phone</label>
                    <input value={form.phone} onChange={e => setForm(f => ({...f, phone: e.target.value}))} placeholder="+507 6000-0000" style={inputStyle}/>
                  </div>
                  <div>
                    <label style={labelStyle}>Email</label>
                    <input value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))} placeholder="email@domain.com" style={inputStyle}/>
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>Internal Notes</label>
                  <input value={form.notes} onChange={e => setForm(f => ({...f, notes: e.target.value}))} placeholder="Availability, rates, key contacts..." style={inputStyle}/>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <div onClick={submitAdd} style={{ flex: 1, textAlign: 'center', padding: '11px', background: tone.accent, color: tone.bg, fontFamily: tone.mono, fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', cursor: 'pointer' }}>
                    Save Provider
                  </div>
                  <div onClick={() => setShowAdd(false)} style={{ padding: '11px 14px', border: `1px solid ${tone.line}`, color: tone.mute, fontFamily: tone.mono, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>
                    Cancel
                  </div>
                </div>
              </div>
